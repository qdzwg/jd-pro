<style scoped>
  ...
  .calendar td.richen span {
    background-color: #529cff;
    color: #fff;
  }
  .calendar td.richen.disabled span {
    background-color: transparent;
    color: #ccc;
  }

  .calendar td.myapply span{
    background-color: #fff;
    color: #ffab2e;
    border: 1px solid #ffab2e;
  }
  .calendar td.myapply.disabled  span{
    background-color: transparent;
    color: #ccc;
    border: 0;
  }

  .calendar td.myon span{
    background-color: #ffab2e;
    color: #fff;
  }
  .calendar td.myon.disabled span{
    background-color: transparent;
    color: #ccc;
  }


   .calendar td.myback span{
    background-color: rgb(21, 224, 14);
    color: #fff;
    /* border: 1px solid #ffab2e; */
  }
  .calendar td.myback.disabled  span{
    background-color: transparent;
    color: #ccc;
    border: 0;
  }

  .calendar td.start span {
  ...
  <div class="calendar">
    <div class="calendar-tools">
            <span class="calendar-prev" @click="prev">
                <svg width="16" height="16" viewBox="0 0 22 22" version="1.1" xmlns="http://www.w3.org/2000/svg"
      ...
      <div class="calendar-info" @click.stop="changeYear">
        ...
        <div class="month">
          <div class="month-inner" :style="{'top':-(this.month*2.4)+'rem'}">
            <span v-for="(m,index) in months" :key="index">{{m}}</span>
          </div>
        </div>
      </div>
      <div class="today" @click="setToday">{{ todayWord }}</div>
    </div>
    <table cellpadding="5" class="calendar-table">
      ...
      <tr v-for="(day,k1) in days" :key="k1" style="{'animation-delay',(k1*30)+'ms'}">
        <td v-for="(child,k2) in day" :key="k2"
            :class="{'selected':child.selected,'disabled':child.disabled,'start':child.start,'end':child.end,'middle':child.middle,'myapply':child.myapply,'myon':child.myon, 'myback':child.myback,'richen':child.richen,'splan':child.splan,'son':child.son}"
            @click="select(k1,k2,$event)"
            :myapply="child.myapply"
            :myon="child.myon"
            :myback="child.myback"
            :richen="child.richen"
            :son="child.son"
            :splan="child.splan" >
          <!--{{child}}-->
          <span v-if="child.myapply" :myapply="child.myapply">审</span>
          <span v-else-if="child.myon" :myon="child.myon">假</span>
          <span v-else-if="child.myback" :myback="child.myback">审</span>
          <span v-else-if="child.richen" :richen="child.richen">{{child.day}}</span>
          <span v-else-if="child.son" :on="child.son">团</span>
          <span v-else-if="child.splan" :splan="child.splan">班</span>
          <span v-else>{{child.day}}</span>
  ...
  export default {
    ...
    data() {
      return {
        ...
        rangeBegin: [],
        rangeEnd: [],
        myDay:this.myLoadDay
      }
    },
    watch: {
      events() {
        this.render(this.year, this.month, this.myDay)
      },
      value() {
        this.init();
      },
      myLoadDay(val,oldVal){
        if(val!=oldVal){
            console.log(val)
            this.myDay = val;
            this.render(this.year, this.month, this.myDay)
    ...
    methods: {
        ...
        for (i = 1; i <= lastDateOfMonth; i++) {
          ...
          if (this.range) { // 范围
            ...
            // 没有默认值的时候显示选中今天日期
              ...
              temp[line].push(Object.assign(
                {day: i, selected: true,myapply:false ,myon:false,richen:false,myback:false,splan:false ,son:false},
                this.getLunarInfo(this.year, this.month + 1, i),
                this.getEvents(this.year, this.month + 1, i),
              ));
              //今天的状态
              for(let myindex=0;myindex<myDay.vacation.length;myindex++){
                for(let ins=0;ins<temp[line].length;ins++) {
                  if (i == myDay.vacation[myindex].day && temp[line][ins].day == i) {
                    switch (myDay.vacation[myindex].status) {
                      case "apply":
                        temp[line][ins].myapply = true;
                        break;
                      case "on":
                        temp[line][ins].myon = true;
                        break;
                    case "back":
                        temp[line][ins].myback = true;
                        break;
                    }
                  }
                }
              }
              for(let myindex=0;myindex<myDay.plans.length;myindex++){
                let day = new Date(myDay.plans[myindex].dateLine).getDate();
                for(let ins=0;ins<temp[line].length;ins++) {
                  console.log("temp",temp[line][ins].day);
                  if (i ==day && temp[line][ins].day == i) {
                    switch (myDay.plans[myindex].status) {
                      case "plan":
                        temp[line][ins].splan = true;
                        break;
                      case "on":
                        temp[line][ins].son = true;
                        break;
                    }
                  }
                }
              ...
              let options = Object.assign(
                {day: i, selected: false ,myapply:false ,myon:false,richen:false,myback:false},
                this.getLunarInfo(this.year, this.month + 1, i),
                this.getEvents(this.year, this.month + 1, i),
              )
              if (this.begin.length > 0) {
              ...
              //根据数据给页面添加属性
              for(let myindex=0;myindex<myDay.vacation.length;myindex++){
                if(i==myDay.vacation[myindex].day){
                  switch (myDay.vacation[myindex].status){
                    case "apply":
                      options.myapply = true;
                      break;
                    case "on":
                      options.myon = true;
                      break;
              ...
              for(let myindex=0;myindex<myDay.plans.length;myindex++){
                ...
                if(i==day){
                  //options.richen = true;
                  /*console.log("status",myDay.plans[myindex].status);
                  console.log("day",day);*/
                  switch (myDay.plans[myindex].status){
                    case "plan":
                      options.splan = true;
                    ...
                    case "on":
                      options.son = true;
                      // options.name = myDay.plans[myindex].routeName;
                      break;
                  }                  
                  // console.log(myDay.plans[myindex]);               
                }
              }
      ...
      select(k1, k2, e) {
        ...
        if (this.range) {
          ...
          this.today = [k1, k2]
          this.$emit('select', [this.year, this.zero ? this.zeroPad(this.month + 1) : this.month + 1, this.zero ? this.zeroPad(this.days[k1][k2].day) : this.days[k1][k2].day],e)
        }
      },
      changeYear() {
        if (this.yearsShow) {
      ...
      selectYear(value) {
        this.yearsShow = false
        this.year = value
        this.$emit('selectYear', this.month+1, this.year)
      },
      // 返回今天
      setToday() {
        let now = new Date();
        ...
        this.days.forEach(v => {
         ...
         if (day != undefined && day.disabled == undefined) {
           day.selected = true
         }
       })
        this.$emit('setToday', this.day,this.month+1, this.year)
        this.$forceUpdate()
      },
      // 日期补零
      zeroPad(n) {
