    ...
    <table cellpadding="5" class="calendar-table">
      ...
      <tr v-for="(day,k1) in days" :key="k1" style="{'animation-delay',(k1*30)+'ms'}">
        <td v-for="(child,k2) in day" :key="k2"
            :class="{'selected':child.selected,'disabled':child.disabled,'start':child.start,'end':child.end,'middle':child.middle,'myon':child.myon,'richen':child.richen}"
            @click="select(k1,k2,$event)"
            :myon="child.myon"
            :richen="child.richen">
          <span v-if="child.myon" :myon="child.myon">假</span>
        ...
        for (i = 1; i <= lastDateOfMonth; i++) {
          ...
          if (this.range) { // 范围
            ...
            // 没有默认值的时候显示选中今天日期
              ...
              temp[line].push(Object.assign(
                {day: i, selected: true,myapply:false ,myon:false,richen:false},
                this.getLunarInfo(this.year, this.month + 1, i),
                this.getEvents(this.year, this.month + 1, i),
              ))
              //今天的状态
              ...
              let options = Object.assign(
                {day: i, selected: false ,myapply:false ,myon:false,richen:false},
                this.getLunarInfo(this.year, this.month + 1, i),
                this.getEvents(this.year, this.month + 1, i),
              )
              if (this.begin.length > 0) {
